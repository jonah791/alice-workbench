 ...
 *
 *  总线里**只有主脑自己活着、且没有待办**时，界面必须教人「怎么开始」，
 *  而不是展示一片空网格——空系统暴露的不是数据问题，是**引导缺失**。
 *
 *  阈值取 `online <= 1`（而非 ≤2）：**≥2 个活节点时星图本身就有信息量了**
 *  （两颗星 + 内环），此时叠一张大卡片会**挡住刚出现的星**——实测踩过：
 *  第二个节点上线后，新星正落在卡片背后，看得见的进度被挡住。
 *  纯函数 ⇒ 可离线单测；阈值只此一处。 */
export function isSparse(stats: StarMapStats): boolean {
  return stats.online <= 1 && stats.openTasks === 0;
}
