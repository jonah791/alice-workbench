...
| 2026-09-14 | **危险动作二次确认落地**（关闭未决 U1） | 一键恢复会调 `init-dsh.ps1`——它第一步清理现有 DSH 进程，等于重启承载会话的运行时。实现为**首次点击进入确认态**（按钮变「确认恢复？再点一次」+ 警示色，5 秒无操作自动撤回；不弹窗、不打断流程）。实测三步证据：文案变化 ✓ / 样式变化 ✓ / 三条本机事件按序进行为流 ✓ |
...
| U8 | 窗口位置持久化 | 每次启动都回到默认位置；实测窗口可能被最小化/移出屏幕（`-21333,-21333`），恢复逻辑未做 |
