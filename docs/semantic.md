...
| **A1b** | **任务一句话列表**（spec P1-1 的末项） | **部分验证** | 数据层：`scan_tasks` 4 条单测（真实台账结构 / 坏文件跳过 / 未收尾优先排序 / 指纹联动）**16/16 绿**；UI：浏览器实机预览渲染正确（「1 未收尾 / 2 · 1 有待验证项」+ 两条一句话列表 + 未验证项高亮）。⚠ **真实总线数据 → Tauri 窗口这一跳我没能截图确认**（WebView2 内容截取在本环境不可靠），归主人目视 |
| **A1c** | **任务的「执行过程」可见**（节点在做什么，spec §5.2） | **部分验证** | 数据层：`scan_actions` 4 条单测（真实 6 行样本解析 / **taskId 回填到无 taskId 的 stage 行** / 坏行跳过 / 指纹联动）**16/16 绿**；UI：`TaskBoard` 展开显示分阶段时间线（`▸` start · `·` stage · `✓` done · `✕` failed，带时钟）。**诚实边界**：思考不落盘 ⇒ 只显示「在做什么」，不显示「在想什么」。⚠ 与 A1b 同：真实数据→窗口那一跳归主人目视 |
